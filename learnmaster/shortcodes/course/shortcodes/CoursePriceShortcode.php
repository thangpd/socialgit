...
class CoursePriceShortcode extends Shortcode {
	const SHORTCODE_ID = 'lema_coursecard_price';
	const PRICE_TYPE_REGULAR = 'course_price_regular';
	const PRICE_TYPE_SALE = 'course_price_sale';
	const PRICE_TYPE_BOTH = 'course_price_both';

	public $contentView = 'price';

	public function _init() {
		lema()->wp->add_filter( 'lema_course_footer', [ $this, 'register' ] );
	}
	...
	public function getShortcodeContent( $data = [], $params = [], $key = '' ) {
		...
		$regular_price = lema()->wp->get_post_meta( $postId, CourseModel::COURSE_REGULAR_PRICE, true );
		if ( ! is_numeric( $regular_price ) ) {
			$regular_price = 0;
		}
		$data['price']['regular'] = $regular_price;

		$sale_price = lema()->wp->get_post_meta( $postId, CourseModel::COURSE_SALE_PRICE, true );
		if ( is_numeric( $sale_price ) ) {
			$data['price']['sale'] = $sale_price;
			$saleTimeLeft          = get_post_meta( $postId, 'course_price_expire', true );
			if ( $saleTimeLeft ) {
