...
namespace lema\core\components\metaBox;
...
class AT_Meta_Box {
	...
	public function __construct( $meta_box ) {
		...
		if ( isset( $meta_box['use_with_theme'] ) ) {
			if ( $meta_box['use_with_theme'] === true ) {
				$this->SelfPath = get_stylesheet_directory_uri() . '/metaBox';
			} elseif ( $meta_box['use_with_theme'] === false ) {
				$this->SelfPath = plugins_url( 'metaBox', plugin_basename( dirname( __FILE__ ) ) );
			} else {
	...
	public function check_field_select() {
		...
		wp_enqueue_style( 'select2', $plugin_path . '/js/select2/select2.css', array(), null );
		wp_enqueue_script( 'select2', $plugin_path . '/js/select2/select2.js', array( 'jquery' ), false, true );
	}
	...
	public function check_field_date() {
		...
		if ( $this->has_field( 'date' ) && $this->is_edit_page() ) {
			// Enqueu JQuery UI, use proper version.
			$plugin_path = $this->SelfPath;
			wp_enqueue_style( 'jquery-ui', $plugin_path . '/js/jquery-ui/jquery-ui.css' );
	...
	public function check_field_time() {
		...
		if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {
			...
			wp_enqueue_style( 'jquery-ui', $plugin_path . '/js/jquery-ui/jquery-ui.css' );
			wp_enqueue_script( 'jquery-ui' );
			wp_enqueue_script( 'timepicker', $plugin_path . '/js/jquery-ui/jquery-ui-timepicker-addon.js', array(
				'jquery-ui-slider',
	...
	public function check_field_code() {
		...
		if ( $this->has_field( 'code' ) && $this->is_edit_page() ) {
			...
			wp_enqueue_style( 'codemirror-dark', $plugin_path . '/js/codemirror/solarizedDark.css', array(), null );
			wp_enqueue_style( 'codemirror-light', $plugin_path . '/js/codemirror/solarizedLight.css', array(), null );
			wp_enqueue_script( 'codemirror', $plugin_path . '/js/codemirror/codemirror.js', array( 'jquery' ), false, true );
			wp_enqueue_script( 'codemirror-js-xml', $plugin_path . '/js/codemirror/xml.js', array( 'jquery' ), false, true );
			wp_enqueue_script( 'codemirror-js-javascript', $plugin_path . '/js/codemirror/javascript.js', array( 'jquery' ), false, true );
			wp_enqueue_script( 'codemirror-js-css', $plugin_path . '/js/codemirror/css.js', array( 'jquery' ), false, true );
			wp_enqueue_script( 'codemirror-js-clike', $plugin_path . '/js/codemirror/clike.js', array( 'jquery' ), false, true );
			wp_enqueue_script( 'codemirror-js-php', $plugin_path . '/js/codemirror/php.js', array( 'jquery' ), false, true );
	...
	public function show_field_repeater( $field, $meta ) {
        ...
        <span class="re-control"><img src="' . $plugin_path . '/images/remove.png" alt="' . __( 'Remove', 'lema' ) . '" title="' . __( 'Remove', 'lema' ) . '" id="remove-' . $field['id'] . '"></span>
		...
		echo '" alt="' . __( 'Add', 'lema' ) . '" title="' . __( 'Add', 'lema' ) . '" id="add-' . $field['id'] . '"><br/></div>';
		...
		echo '</table><img src="' . $plugin_path . '/images/remove.png" alt="' . __( 'Remove', 'lema' ) . '" title="' . __( 'Remove', 'lema' ) . '" id="remove-' . $field['id'] . '"></div>';
	...
	public function show_field_posts( $field, $meta ) {
		...
		if ( 'checkbox_list' == $options['type'] ) {
			...
			if ( isset( $field['emptylabel'] ) ) {
				echo '<option value="-1">' . ( isset( $field['emptylabel'] ) ? $field['emptylabel'] : __( 'Select ...', 'lema' ) ) . '</option>';
			}
			foreach ( $posts as $p ) {
